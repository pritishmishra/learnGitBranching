...
DEFAULT_LOGFILE = "/var/log/learngit/lesson_data.jsonl"
...
def build_report(path, mode):
    ...
    for sid, record in selected.items():
        rows.append({
            "student_id": sid,
            "count": record["count"],
            "completed_lessons": ";".join(record["completed"]),
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a CSV report from Learn Git lesson data submissions."
    )
    parser.add_argument(
        "logfile",
        nargs="?",
        default=DEFAULT_LOGFILE,
        help="Path to lesson_data.jsonl. Use '-' for stdin.",
    )
    parser.add_argument(
        "--mode",
        choices=["best", "latest"],
